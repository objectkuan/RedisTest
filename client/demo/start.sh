...
for (( i=0; i<$INSNUM; i++ )); do
	...
	for (( j=0; j<$TIMES; j++ )); do
		...
		echo "Demo instance amount: $INSNUM" >> results/result-$port-$j
		echo "Parallel client amount: $PARAL" >> results/result-$port-$j
		echo "" >> results/result-$port-$j
		echo "" >> results/result-$port-$j
		ab -n 1000000 -c $PARAL http://$SERVER:$port/ >> results/result-$port-$j 2>&1 &
		pids[${#pids[@]}]=$!
		echo "ab -n 1000000 -c 100 http://$SERVER:$port/ >> results/result-$port-$j 2>&1 &"
	done;
done;
